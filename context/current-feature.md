...
- 2026-07-08 (social + chrome) —
  - **Social sharing image**: replaced the SVG OG (which social platforms don't
    render, and which listed areas + "phones/plans") with a branded **PNG**
    `/public/og.png` (1200×630, fibre-only, no areas), generated by rendering an
    HTML card via headless browser. `buildMetadata` now defaults to it and
    forces a raster fallback for any non-raster image. Blog posts use their photo.
  - **Areas**: audited whole repo — removed the last area strings (the OG image).
    Kept the coverage form's State dropdown (functional) and "Malaysia" (country).
  - **Fibre-only**: softened the 5G mention in the fibre-vs-wifi guide.
  - **"Sofie" → "us"** everywhere ("Chat with us"); removed `site.agentName`.
    Default WhatsApp text: "Hi, I'm interested in a Maxis Home Fibre plan…".
  - **Floating WhatsApp button** on every page (`FloatingWhatsApp`), Maxis Shock
    Green with pulse-ring + idle-bob animation (respects reduced-motion).
  - **Navbar**: removed the "WhatsApp us" button (floating one covers it),
    made it taller, added a Shock-Green top border.
